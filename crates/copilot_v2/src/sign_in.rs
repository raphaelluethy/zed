use crate::{CopilotV2, CopilotV2Status, request::PromptUserDeviceFlow};
use gpui::{
...
pub fn initiate_sign_in(window: &mut Window, cx: &mut App) {
    let Some(copilot) = CopilotV2::global(cx) else {
        return;
    };
    ...
    match copilot.read(cx).status() {
        CopilotV2Status::Starting { task } => {
            ...
            cx.spawn_in(window, async move |workspace, cx| {
                task.await;
                if let Some(copilot) = cx
                    .update(|_window, cx| CopilotV2::global(cx))
                    .ok()
                    .flatten()
                {
                    workspace
                        .update_in(cx, |workspace, window, cx| {
                            match copilot.read(cx).status() {
                                CopilotV2Status::Authorized => workspace.show_toast(
                                    Toast::new(
                                        NotificationId::unique::<CopilotStatusToast>(),
                                        "CopilotV2 has started.",
                                    ),
                                    cx,
                                ),
                                _ => {
...
impl ModalView for CopilotCodeVerification {
    fn on_before_dismiss(
        ...
        self.copilot.update(cx, |copilot, cx| {
            if matches!(copilot.status(), CopilotV2Status::SigningIn { .. }) {
...
impl CopilotCodeVerification {
    pub fn new(copilot: &Entity<CopilotV2>, cx: &mut Context<Self>) -> Self {
        ...
        Self {
            ...
            _subscription: cx.observe(copilot, |this, copilot, cx| {
                ...
                match status {
                    ...
                    | CopilotV2Status::Unauthorized
                    | CopilotV2Status::SigningIn { .. } => this.set_status(status, cx),
    ...
    fn render_prompting_modal(
        ...
        v_flex()
            ...
            .gap_2()
            .items_center()
            .child(Headline::new("Use GitHub CopilotV2 in Zed.").size(HeadlineSize::Large))
            .child(
                Label::new("Using CopilotV2 requires an active subscription on GitHub.")
                    .color(Color::Muted),
            )
    ...
    fn render_enabled_modal(cx: &mut Context<Self>) -> impl Element {
        v_flex()
            .gap_2()
            .child(Headline::new("CopilotV2 Enabled!").size(HeadlineSize::Large))
            .child(Label::new(
                "You can update your settings or sign out from the CopilotV2 menu in the status bar.",
            ))
            .child(
    ...
    fn render_unauthorized_modal(cx: &mut Context<Self>) -> impl Element {
        v_flex()
            .child(Headline::new("You must have an active GitHub CopilotV2 subscription.").size(HeadlineSize::Large))

            .child(Label::new(
                "You can enable CopilotV2 by connecting your existing license once you have subscribed or renewed your subscription.",
            ).color(Color::Warning))
            .child(
...
impl Render for CopilotCodeVerification {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let prompt = match &self.status {
            CopilotV2Status::SigningIn { prompt: None } => {
