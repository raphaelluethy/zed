...
impl CopilotV2Provider {
    pub fn new(copilot: Entity<CopilotV2>) -> Self {
        Self {
            cycled: false,
            buffer_id: None,
            completions: Vec::new(),
        ...
        self.pending_refresh = Some(cx.spawn(async move |this, cx| {
            if debounce {
                cx.background_executor()
                    .timer(COPILOT_V2_DEBOUNCE_TIMEOUT)
                    .await;
            }
...
mod tests {
    ...
    async fn test_copilot(executor: BackgroundExecutor, cx: &mut TestAppContext) {
        ...
        let (copilot, copilot_lsp) = CopilotV2::fake(cx);
        ...
        executor.advance_clock(COPILOT_V2_DEBOUNCE_TIMEOUT);
        cx.update_editor(|editor, window, cx| {
